<div class="timeline">

  <div class="tl-item">
    <a class="tl-logo" href="https://www.stepfun.com" target="_blank" rel="noopener">
      <img loading="lazy" decoding="async" src="static/assets/img/logos/stepfun.png" alt="StepFun" />
    </a>
    <div class="tl-card">
      <p class="tl-role">基础模型研究实习生 · <span class="tl-org">阶跃星辰 (StepFun)</span></p>
      <p class="tl-meta">2026 年 5 月 - 至今 · 上海</p>
      <ul>
        <li>参与 <strong>Agentic Rubrics</strong> RL 后训练，以及面向复杂任务的内部智能体评测基准建设。</li>
        <li>基于 <strong>Claude Code</strong> 和 <strong>Codex</strong> 搭建了自动化 rubrics-benchmark 生成流水线，包括自动生成、质量控制和自迭代。</li>
        <li>在内网部署了 <strong>Hermes</strong> 和 <strong>Openclaw</strong>，将智能体能力集成到团队日常工作流中。</li>
      </ul>
    </div>
  </div>

  <div class="tl-item">
    <a class="tl-logo" href="https://www.longcatai.org" target="_blank" rel="noopener">
      <img loading="lazy" decoding="async" src="static/assets/img/logos/longcat.png" alt="Meituan LongCat" />
    </a>
    <div class="tl-card">
      <p class="tl-role">基础模型研究实习生 · <span class="tl-org">美团 · 龙猫 (Keeta · LongCat)</span></p>
      <p class="tl-meta">2025 年 9 月 - 2025 年 12 月 · 上海</p>
      <ul>
        <li>部署并后训练了<strong>视觉语言模型 (VLM)</strong> 和<strong>视觉语言动作模型 (VLA)</strong>，在内部平台上线。</li>
        <li>研究并实现了提升多模态大模型在游戏场景下表现的<strong>智能体策略</strong>。</li>
        <li>共一作者发表 <strong>SOP-Maze</strong>（ACL Findings 2026），另有两篇 LLM 指令遵循评测方向的投稿。</li>
      </ul>
    </div>
  </div>

</div>
